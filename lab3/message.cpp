...
while(true) {
    ...
    if(m.queue_id == 1){
        ...
        buffer1.push_front(m);//dodaj element na przód kolejki
        ...
        std::this_thread::sleep_for(std::chrono::seconds(4));//produkcja trwa 4 sekundy
        ...
        empty_q2.wait();
        mutex_q2.lock();//semafor empty i mutex bufora zablokowany

        buffer2.push_front(m);//dodaj elelent na przód

        mutex_stream.lock();//mutex wyjścia zablokowany
        std::cout<<"producent "<<m.producer_id<<" produced msg: "<<m.message_id<<"to queue: " << m.queue_id<<std::endl;
        std::this_thread::sleep_for(std::chrono::seconds(4));//produkcja trwa 4 sekundy
        printBuffers();//wypisz kolejki
        mutex_stream.unlock();
...
void Synchronization::consume() {
    ...
    while(true) {
        ...
        full_q1.wait();
        mutex_q1.lock();//semafor full i mutex bufora zablokowany, usuń wiadomość z tyłu kolejki

        buffer1.pop_back();

        mutex_q1.unlock();//odblokuj bufor
        empty_q1.signal();//pusty++

        mutex_stream.lock();//strumień wyjścia zablokowany wypisz informację i aktualny rozmiar bufora
        std::cout<<"deleted msg from first que. current size "<< buffer1.size()<<std::endl;
        mutex_stream.unlock();
        }  
        if((!buffer2.empty()) /*&& (buffer1.size() < S)*/) {
        full_q2.wait();
        mutex_q2.lock();//semafor i mutex bufora zablokowany

        buffer2.pop_back();//usuń wiadomość z tyłu kolejki

        mutex_q2.unlock();//odblokuj bufor
        empty_q2.signal();

        mutex_stream.lock();//strumień wyjścia zablkowany wypisz informację
